...
class Vector2D{
  ...
  double dot_product(Vector2D *v){
    return this->x * v->x + this->y * this->y * v->y;
  }
  double magnitude(){
...
//** as part of TODO: declaring get_closest_point_index
/*std::size_t get_closest_point_index(double point_x,double point_y, std::vector<double> points_x, std::vector<double> points_y){
  //Index of closes point to be found
  std::size_t closest_index = 0;

  //calculating distance to point
  std::size_t dist_min = std::numeric_limits<double>::infinity();

  //Find closest point in vector
  for(std::size_t i=0;i < points_x.size(); ++i){
    double dist = std::pow((std::pow((point_x-points_x[i]),2) + 
                            + std::pow((point_y - points_y[i]), 2)), 0.5);
    if (dist < dist_min){
      dist_min = dist;
      closest_index = i;
    }
...
int main ()
{
  ...
  **/
  ...
  //initialize values
  // First case: P-gain only
  //pid_steer.Init(1.0, 0.0, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.5, 0.0, 0.0, 1.2, -1.2);
  
  // Second case: PD gain only
  //pid_steer.Init(1.0, 0.0, 0.0, 1.2, -1.2);
  //pid_steer.Init(0.5, 0.001, 0.0, 1.2, -1.2);
  ...
  //pid_steer.Init(0.4, 0.01, 0.8, 0.61, -0.61);
  //pid_steer.Init(0.1, 0.0, 0.0125, 0.61, -0.61);
  //pid_steer.Init(0.3, 0.01, 0.4, max_steer, -max_steer);
  //pid_steer.Init(0.25, 0.1, 0.5, max_steer, -max_steer);
  pid_steer.Init(0.3, 0.0025, 0.17, 0.60, -0.60);
  ...
  **/
  ...
  //pid_throttle.Init(0.5, 0.0, 0.0, 1.0, -1.0);
  
  // Second case: PD gain only
  //pid_throttle.Init(1.0, 0.0, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.5, 0.001, 0.0, 1.0, -1.0);

  // Third case: PID gain only
  //pid_throttle.Init(1.0, 0.0, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.5, 0.001, 0.0, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0009, 0.1, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0006, 0.08, 1.0, -1.0);
  //pid_throttle.Init(0.21, 0.0009, 0.1, 1.0, -1.0);
  //pid_throttle.Init(2.0, 0.8, 0.1, 1.0, -1.0);
  //pid_throttle.Init(0.35, 0.01, 0.2, max_throttle, -max_break);
  

  //pid_throttle.Init(0.25, 0.05, 0.1, max_throttle, max_break);
  pid_throttle.Init(0.21, 0.0006, 0.080, max_throttle, max_break);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          pid_steer.UpdateDeltaTime(new_delta_time); //uncommented to update delta time variable in correspondence to previous steering command
   
          // We get the index of the closest point in the reference trajectory of the car
          //std::size_t index_closest_point = get_closest_point_index(x_position,y_position,x_points,y_points);
         
          // Compute steer error
          double error_steer;
          double steer_output;
          //error_steer = angle_between_points(x_position,y_position,x_points[index_closest_point],y_points[index_closest_point] - yaw);
          error_steer = 0;
          ...
          **/
//           // Update the delta time with the previous command
          pid_throttle.UpdateDeltaTime(new_delta_time);   //uncommented to update the delta time with respect to the previous command

          // Compute error of speed
          double error_throttle;
          //error_throttle = v_points[index_closest_point]-velocity; //to calculate the distance between current and trajectory velocity
...
//           // Compute control to apply
           ...
           //double throttle_response = pid_throttle.TotalError();
