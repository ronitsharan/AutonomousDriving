...
#include "velocity_profile_generator.h" //added
...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  //auto look_ahead_distance = (0.5*accel_mag*std::pow(_lookahead_time,2)
    //                         + velocity_mag*_lookahead_time);  // <- Fix This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = 0.0;  // Goal speed for stopping
      goal.velocity.y = 0.0;  
      goal.velocity.z = 0.0;  
      ...
      goal.velocity.x = _speed_limit*std::cos(goal.rotation.yaw);  // according to pre-defined speed limit as mentioned above
      goal.velocity.y = _speed_limit*std::sin(goal.rotation.yaw);  // according to pre-defined speed limit as mentioned above
      goal.velocity.z = 0;
    }
